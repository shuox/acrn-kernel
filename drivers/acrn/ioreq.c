...
static int ioreq_task(void *data)
{
	...
	if (!client->handler)
		return 0;
	ioreqs = client->ioreqs_map;
	while (!kthread_should_stop()) {
		while (has_pending_request(client)) {
			vcpu = find_first_bit(ioreqs, client->vm->vcpu_num);
		...
		bool is_default, char *name)
{
	...
	client->thread = kthread_run(ioreq_task, client, "VM%ld-%s",
...
void acrn_ioreq_clear_request(struct acrn_vm *vm)
{
	...
	spin_lock_bh(&vm->ioreq_clients_lock);
	client = list_prev_entry(&vm->ioreq_clients, list);
...
{
	write_lock_bh(&ioreq_clients_lock);
	list_del_init(&client->list);
	write_unlock_bh(&ioreq_clients_lock);
	...
	if (client->is_default)
		...
		/* Other clients, just stop their threads */
 ...
 * clients should take care itself
 */
			...
			uint32_t type, long start, long end)
{
	...
	if (end < start) {
		pr_err("acrn: invalid IO range [0x%x,0x%x]\n", start, end);
		return -EFAULT;
	}
...
static struct ioreq_client *client get_default_client(struct acrn_vm *vm)
{
...
int acrn_ioreq_attach_client(struct ioreq_cliend *client)
{
	/* only default client could attach */
	if (!client->is_default)
		return 0;
	wait_event_interruptible(client->wq,
...
static int handle_cf8cfc(struct acrn_vm *vm, struct acrn_request *req, int vcpu)
{
	...
	if (is_handled)
		ret = ioreq_complete_request(vm->vmid, vcpu, req);

	return ret ? ret: is_handled;
}
...
{
	struct list_head *pos, *tmp;
	struct ioreq_client *pos, *next;

	/* Destroy all clients belong to this VM */
	list_for_each_entry_safe(pos, next, &vm->ioreq_clients, list)
